/* ==========================================================================
   Interactive Engine for EduBeacon
   ========================================================================== */

// Base Simulated Article Database
const localArticles = [
    {
        "title": "Bootcamps vs. Degrees: Navigating the Tech Job Market",
        "excerpt": "With software engineering jobs shifting, hiring managers are prioritizing portfolio projects over college names.",
        "category": "Career Advice",
        "author": "Dr. Sarah Miller",
        "date": "2 hours ago",
        "readTime": "5 min read",
        "body": "<p>For individuals looking to transition into software engineering, deciding between a coding bootcamp and a traditional computer science degree is a key crossroad.</p><p><b>Coding Bootcamps</b> offer intensive, project-focused training designed to build practical web development skills in 3 to 6 months. They are ideal for self-directed learners looking to build real-world portfolios quickly. However, they lack theoretical computer science coverage, which can limit options for systems engineering roles.</p><p><b>Computer Science Degrees</b> provide foundational coverage of algorithms, systems architecture, and discrete mathematics. While highly valued by enterprise employers, a 4-year degree requires significant financial and time commitments, and may not cover modern framework development.</p>",
        "premium": true
    },
    {
        "title": "The Rise of AI Tutoring Companions in K-12 Classrooms",
        "excerpt": "New educational initiatives demonstrate how automated tutoring modules support student progression in math and reading.",
        "category": "Ed-Tech",
        "author": "David Rostova",
        "date": "4 hours ago",
        "readTime": "4 min read",
        "body": "<p>Integrating AI-driven tutoring companions into classrooms is providing teachers with powerful tools to support student progression.</p><p>These automated tutoring systems analyze student responses in real time, adapting exercise difficulty and explaining concepts based on individual learning profiles. In trial programs, schools deploying AI helpers reported significant increases in student test scores.</p><p>Educators emphasize that AI is designed to support, not replace, human teachers. It handles repetitive exercises and data tracking, freeing teachers to focus on student emotional and social support.</p>",
        "premium": false
    },
    {
        "title": "Micro-Credentials: The Evolving Face of Professional Training",
        "excerpt": "Discover how bite-sized certifications are replacing traditional academic degrees for career transitions in digital marketing and tech.",
        "category": "Certifications",
        "author": "Meera Joshi",
        "date": "1 day ago",
        "readTime": "5 min read",
        "body": "<p>Traditional degrees are no longer the exclusive path to career advancement. Micro-credentials\u2014short, targeted courses focusing on specific technical skills\u2014are gaining rapid adoption among employers.</p><p>Major technology and advertising platforms are collaborating with universities to offer micro-credentials in data analytics, cloud architecture, and marketing metrics. These certifications are cost-effective, self-paced, and focus on immediate practical application.</p>",
        "premium": true
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isArticlePage) {
        if (artId !== null) {
            ...
            const savedRssArt = localStorage.getItem('5_edu_beacon_active_rss_article');
            const savedRssList = localStorage.getItem('5_edu_beacon_rss_articles');
            if (savedRssList) {
...
function loadStateFromStorage() {
    const savedTheme = localStorage.getItem('5_edu_beacon_dark_mode');
    state.isDarkMode = savedTheme === 'true';

    const savedBookmarks = localStorage.getItem('5_edu_beacon_bookmarks');
    if (savedBookmarks) {
        state.bookmarks = JSON.parse(savedBookmarks);
    }

    const savedUpvotes = localStorage.getItem('5_edu_beacon_upvotes');
    if (savedUpvotes) {
        state.upvoted = JSON.parse(savedUpvotes);
    }

    const savedComments = localStorage.getItem('5_edu_beacon_comments');
    if (savedComments) {
        state.comments = JSON.parse(savedComments);
    }

    const savedSub = localStorage.getItem('5_edu_beacon_subscribed');
    state.isSubscribed = savedSub === 'true';

    const savedViews = localStorage.getItem('5_edu_beacon_pageviews');
    if (savedViews) state.pageViews = parseInt(savedViews);

    const savedClicks = localStorage.getItem('5_edu_beacon_adclicks');
    if (savedClicks) state.adClicks = parseInt(savedClicks);

    const savedEarnings = localStorage.getItem('5_edu_beacon_earnings');
...
function saveStateToStorage() {
    localStorage.setItem('5_edu_beacon_dark_mode', state.isDarkMode);
    localStorage.setItem('5_edu_beacon_bookmarks', JSON.stringify(state.bookmarks));
    localStorage.setItem('5_edu_beacon_upvotes', JSON.stringify(state.upvoted));
    localStorage.setItem('5_edu_beacon_comments', JSON.stringify(state.comments));
    localStorage.setItem('5_edu_beacon_subscribed', state.isSubscribed);
    localStorage.setItem('5_edu_beacon_pageviews', state.pageViews);
    localStorage.setItem('5_edu_beacon_adclicks', state.adClicks);
    localStorage.setItem('5_edu_beacon_earnings', state.earnings.toFixed(2));
}
...
function renderTicker() {
    ...
    const items = ["Universities report 35% enrollment increase in asynchronous online master's programs", "Global ed-tech funding transitions toward targeted vocational skills training platforms", "Interactive coding sandboxes become standard features in software engineering programs", "High schools explore AI writing assistants to support literacy education", "Professional certificate earners report average salary gains of 18% within one year"];
...
function handleArticleClick(article) {
    ...
    if (localIdx > -1) {
        ...
        localStorage.setItem('5_edu_beacon_active_rss_article', JSON.stringify(article));
...
function runWorkspaceCalculator() {
    const resultEl = document.getElementById('calc-results');
            ...
            let course = 'Intro to Web Development';
            let duration = '6 Months';
            let salRange = '$70k - $85k';
            
            if (goal === 'webdev') {
                if (exp === 'novice') {
                    course = 'Software Engineering Career Bootcamp';
                    duration = time === 'part' ? '9 Months' : '3 Months';
                } else {
                    course = 'Advanced Full-stack Node/React Certification';
                    duration = '4 Months';
                }
                salRange = '$85,000 - $110,000';
            } else if (goal === 'data') {
                course = 'Python for Data Analysis & SQL Foundations';
                duration = '6 Months';
                salRange = '$90,000 - $120,000';
            } else if (goal === 'cyber') {
                course = 'CompTIA Security+ Prep & Threat Hunting Lab';
                duration = '5 Months';
                salRange = '$80,000 - $105,000';
            } else {
                course = 'Conversion Marketing & SEO Strategy Bootcamp';
                duration = '3 Months';
                salRange = '$65,000 - $85,000';
            }
            
            resultEl.innerHTML = `
                <div class="calc-result-box" style="border-left: 4px solid var(--secondary); padding-left: 10px; margin-top: 15px;">
                    <h5 style="margin: 0 0 5px 0;">Recommended Study Track:</h5>
                    <div style="font-weight: bold; color: var(--secondary); font-size: 15px;">${course}</div>
                    <div style="font-size: 12px; margin-top: 5px;">Study Duration: <strong>${duration}</strong></div>
                    <div style="font-size: 12px;">Est. Starting Salary: <strong style="color: var(--primary);">${salRange}</strong></div>
                </div>
            `;
...
<a href="\${link}" target="_blank" class="hero-btn" style="display:inline-block; margin-top:10px;">Read Original Source <i class="fa-solid fa-external-link"></i></a>`
            ...
            localStorage.setItem('5_edu_beacon_rss_articles', JSON.stringify(parsedArticles));
